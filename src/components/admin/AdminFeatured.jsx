...
export default function AdminFeatured() {
  ...
  async function loadRequests() {
    ...
    try {
      setRequests(await fetchAdminFeaturedRequests(status))
    } catch (loadError) {
  ...
  async function handleReview(requestId, approved) {
    ...
    try {
      await adminReviewFeatured(requestId, approved, reasons[requestId] ?? '')
      await loadRequests()
    } catch (reviewError) {
  ...
  return (
    <div className="admin-panel">
      ...
      {!loading && requests.length === 0 ? (
        <p className="status-message">No featured requests in this view.</p>
      ) : null}
